...
function requestActionsHtml(request) {
  if (request.status === "PENDING") {
    return `
      <button class="request-action-btn match" data-request-action="MATCHED" data-id="${request.id}">تعيين كمطابق</button>
  ...
  if (request.status === "MATCHED") {
    return `
      <button class="request-action-btn complete" data-request-action="COMPLETED" data-id="${request.id}">إكمال</button>
      <button class="request-action-btn cancel" data-request-action="CANCELLED" data-id="${request.id}">إلغاء</button>
    `;
  }
...
function openCriticalWorkflowModal() {
  setPage("workflow");
  openRequestModal();
}
...
function setupNavigation() {
  ...
  qs("#runWorkflowBtn")?.addEventListener("click", openCriticalWorkflowModal);
}
  ...
  qs("#requestsList")?.addEventListener("click", async (event) => {
    ...
    if (viewButton) {
      ...
      alert(
        ...
        `الأولوية: ${translateStatus(request.urgency)}\n` +
        `الحالة: ${translateStatus(request.status)}`
      );
    }
  });
}
