  ...
  if (requestForm && !qs("#doctorName")) {
    ...
    cityField?.closest(".form-group, .field, label, div")?.insertAdjacentHTML("afterend", `
      <div class="modal-grid-2 domain-extra-fields">
        ...
        <div class="form-field">
          <label for="requestQuantity">الكمية المطلوبة</label>
        ...
        <div class="modal">
          <div class="modal-header">
            <div>
              ...
              <p>أدخل بيانات المستلم والكمية المسلمة.</p>
            </div>
          ...
          <form id="handoverForm" class="modal-form">
            ...
            <div class="modal-grid-2">
              ...
              <div class="form-field">
                <label for="deliveredQuantity">الكمية المسلمة</label>
...
function renderHospitalsDomain() {
  ...
  if (!state.hospitals.length) {
    list.innerHTML = `<div class="empty-state">لا توجد مستشفيات.</div>`;
    return;
  }

  list.innerHTML = state.hospitals.map((item) => `
    <div class="domain-row">
      ...
      <div class="domain-cell">${item.isActive ? badge("نشط", "green") : badge("متوقف", "red")}</div>
      <div></div>
    </div>
  `).join("");
}
...
function renderDonationsDomain() {
  ...
  if (!state.donations.length) {
    list.innerHTML = `<div class="empty-state">لا توجد تبرعات مسجلة.</div>`;
    return;
  }

  list.innerHTML = state.donations.map((item) => `
    <div class="domain-row">
      <div class="domain-row-main">
      ...
      <div class="domain-cell">الكمية: ${escapeHtml(item.quantity)}</div>
      <div class="domain-cell">${String(item.donationDate || "").slice(0, 10)}</div>
      <div>${badge(item.status, item.status === "ACCEPTED" ? "green" : "gray")}</div>
    </div>
  `).join("");
}
...
function renderInventoryDomain() {
  ...
  list.innerHTML = state.inventory.map((item) => `
    <article class="domain-card inventory-card-modern">
      ...
      <span>الكمية المتاحة: ${escapeHtml(item.availableQuantity)}</span>
      <span>الكمية المحجوزة: ${escapeHtml(item.reservedQuantity)}</span>
...
function renderHandoversDomain() {
  ...
  list.innerHTML = state.handovers.map((item) => `
    <div class="domain-row">
      ...
      <div class="domain-cell">الكمية: ${escapeHtml(item.deliveredQuantity)}</div>
...
/* =========================================================
   SIDEBAR BRAND HERO
   يضع شعار BloodNet أعلى القائمة الجانبية بنفس الشكل المطلوب
   ========================================================= */
document.addEventListener("DOMContentLoaded", function () {
  ...
  // احذف الشعار المكرر إن وجد
  sidebar.querySelectorAll(".sidebar-brand-hero").forEach(el => el.remove());

  const brandHero = document.createElement("div");
  brandHero.className = "sidebar-brand-hero";
  brandHero.innerHTML = `
    <div class="sidebar-brand-hero__row">
      <div class="sidebar-brand-hero__text">
        <div class="sidebar-brand-hero__title">BloodNet</div>
        <div class="sidebar-brand-hero__subtitle">نظام موزع لإدارة التبرع</div>
      </div>

      <div class="sidebar-brand-hero__icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 3C12 3 6.5 9 6.5 13.5C6.5 16.95 8.95 19.5 12 19.5C15.05 19.5 17.5 16.95 17.5 13.5C17.5 9 12 3 12 3Z"></path>
          <path d="M10.2 13.2C10.45 12.25 11.15 11.45 12.1 11.05"></path>
        </svg>
      </div>
    </div>
  `;

  // ضع الشعار في أعلى القائمة الجانبية
  const firstElement = sidebar.firstElementChild;
  if (firstElement) {
    sidebar.insertBefore(brandHero, firstElement);
  } else {
    sidebar.appendChild(brandHero);
  }
});
